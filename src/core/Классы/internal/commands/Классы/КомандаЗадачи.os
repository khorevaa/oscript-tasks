...
Перем ПеременныеОкруженияПроцесса;
...
Процедура ПерехватыватьПотоки(Знач ППерехватыватьПотоки) Экспорт
	ПерехватыватьПотоки = ППерехватыватьПотоки;
КонецПроцедуры

Процедура УстановитьПеременныеОкружения(Знач ППеременныеОкруженияПроцесса) Экспорт

	Если ППеременныеОкруженияПроцесса = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ПеременныеОкруженияПроцесса = ППеременныеОкруженияПроцесса;
...
Процедура ДобавитьЛогВыводаКоманды(Знач ИмяЛогаИлиЛог) Экспорт
	...
	НовыйЛог.ДобавитьСпособВывода(ЦветнойВывод);
...
Функция ЗапуститьИПодождать()
	...
	ПолучитьВыводПокаПроцессИсполняется(Процесс, ПерехватыватьПотоки);	
...
Функция ЗапуститьПроцессКоманды(ПерехватыватьПотоки = Ложь)
	...
	ПеременныеЗапуска = ПеременныеСреды();

	Если Не ПеременныеОкруженияПроцесса = Неопределено Тогда
		Для каждого КлючЗначение Из ПеременныеОкруженияПроцесса Цикл
			ПеременныеЗапуска.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;
	КонецЕсли;
	...
	Попытка
		...
		Если ВерсияБольшеИлиРавна21 Тогда
			...
			Если КодировкаВывода = Неопределено Тогда
				Процесс = СоздатьПроцесс(СтрокаЗапуска, РабочийКаталог, ПерехватыватьПотоки, ПерехватыватьПотоки, , ПеременныеЗапуска);
			Иначе
				Процесс = СоздатьПроцесс(СтрокаЗапуска, РабочийКаталог, ПерехватыватьПотоки, ПерехватыватьПотоки, КодировкаВывода, ПеременныеЗапуска);
			КонецЕсли;
...
Процедура ПолучитьВыводПокаПроцессИсполняется(Процесс, ПерехватыватьПотоки)
...
Функция ОбработатьОчереднуюСтрокуВывода(ПотокПроцесса, ЗаписьXML, Знач ПрефиксДляОтладки, Знач ЭтоВыводОшибки = Ложь)
	...
	Пока ПотокПроцесса.ЕстьДанные Цикл
		СтрокаПотока = ПотокПроцесса.ПрочитатьСтроку();
		
		Если СтрокаПотока <> "" Тогда
			
			Лог.Отладка("в цикле %2%1", СтрокаПотока, Символы.ПС);
			Если ЗначениеЗаполнено(ПодключенныеЛоги) Тогда
				...
				Для каждого ПодключенныйЛог Из ПодключенныеЛоги Цикл
					Если ЭтоВыводОшибки Тогда
						ПодключенныйЛог.Ошибка(СтрокаПотока);
					Иначе
						ПодключенныйЛог.Информация(СтрокаПотока);
					КонецЕсли;
				КонецЦикла;
			Иначе
				ЗаписьXML.ЗаписатьБезОбработки(СтрокаПотока + Символы.ПС);
	
				Если НемедленнныйВывод Тогда
					Сообщить(СтрокаПотока);
				КонецЕсли;
			КонецЕсли;
...
Процедура Инициализация()
    ...
    Лог = Логирование.ПолучитьЛог(ИмяЛога()).Поля("prefix", "commands");
